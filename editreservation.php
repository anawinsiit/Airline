...
<?php
    ...
    $sql = ("SELECT * FROM reservation WHERE customer_id = $user");
    $detail = ("    SELECT 
                                        ...
                                        <?php
                                            if(!$cus_reserve -> num_rows){
												...
												<tr>
                                                    ...
                                                    <?php
                                                        if ($Rerserve_list['status'] == "WAIT PAY"){
                                                        ?>
													<td>
                                                        <td>
                                                            <form action='pay_form.php' method='POST'>
                                                            <button  type='submit' name='pay' class='btn btn-info btn-md ' value='<?=$Rerserve_list['reservation_id']?>'>
                                                            PAY NOW</button>
                                                            </form>
                                                        </td>
                                                        <td>
                                                            <form action='cancel.php' method='POST'>
                                                                <button  type='submit' name='cancel' class='btn btn-danger btn-md' value='<?=$Rerserve_list['reservation_id']?>'>
                                                                CANCEL</button>
                                                            </form>
                                                        </td>
                                                    </td>
                                                    <?php } 
                                                    else if($Rerserve_list['status'] == "Cancel" or $Rerserve_list['status'] == "Expired"){ ?>
                                                    <td>
                                                        <td>
                                                            <form action='delete.php' method='POST'>
                                                                <button  type='submit' name='delete' class='btn btn-secondary btn-md px-4' value='<?=$Rerserve_list['reservation_id']?>'>
                                                                Delete</button>
                                                            </form>
                                                        </td>
                                                    <?php } ?>
